...
import VideoCategoryForm from '../../components/video-categories/VideoCategoryForm.vue';
import ConfirmModal from '../../components/common/ConfirmModal.vue';
import { useVideoCategoryStore } from '../../stores/videoCategoryStore';

const videoCategoryStore = useVideoCategoryStore();
const { categories, meta, loading, error } = storeToRefs(videoCategoryStore);
...
onMounted(async () => {
    await videoCategoryStore.fetchCategories();
});
...
watch(
    ...
    (value) => {
        ...
        searchTimeout.value = setTimeout(() => {
            videoCategoryStore.fetchCategories({
                search: value,
                page: 1,
            });
        }, 350);
...
async function confirmDelete() {
    ...
    try {
        await videoCategoryStore.deleteCategory(categoryToDelete.value.id, {
            search: searchTerm.value,
            per_page: meta.value.per_page,
        });
        closeDeleteModal();
        ...
        deleting.value = false;
        deleteError.value = videoCategoryStore.error || 'Video kategorisi silinirken bir hata oluştu.';
    }
}
        ...
        >
            <div>
                ...
                <p class="mt-2 text-sm text-neutral-500">
                    Videoları kategori bazında düzenleyin, listelemeleri daha anlaşılır hale getirin.
                </p>
            </div>
        ...
        <div class="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-neutral-100">
                        <thead class="bg-neutral-50/80">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">
                                    Adı
                                </th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">
                                    Slug
                                ...
                                <th scope="col" class="px-6 py-4 text-right text-xs font-semibold uppercase tracking-[0.14em] text-neutral-400">
                                    İşlemler
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-neutral-100">
                            ...
                            <tr v-else-if="!hasCategories">
                                <td colspan="5" class="px-6 py-8 text-center text-sm text-neutral-500">
                                    Henüz video kategorisi eklenmemiş. İlk kategori için sağ üstteki butonu kullanın.
                                </td>
                            </tr>
                            <tr v-else v-for="category in categories" :key="category.id" class="hover:bg-neutral-50/60 transition">
                                ...
                                <td class="whitespace-nowrap px-6 py-5 text-right">
                                    <div class="inline-flex gap-3 text-sm font-semibold">
                                        <button
                                            type="button"
            ...
            >
                ...
                <p class="mt-4 text-center">
                    Yeni bir video kategorisi oluşturmak için sağ üstteki butonu kullanın.
                </p>
            </div>
        </div>
