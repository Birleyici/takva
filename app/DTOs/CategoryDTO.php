...
class CategoryDTO
{
    ...
    /**
     * @param \Illuminate\Support\Collection<int, Category>|\Illuminate\Support\Collection<int, array<string, mixed>> $categories
     * @return array<int, CategoryDTO>
     */
    public static function collection(Collection $categories): array
    {
        return $categories->map(fn ($category) => $category instanceof Category
            ? self::fromModel($category)
            : self::fromArray($category)
        )->all();
    }
